...
// Input-> parsing-> assign cell properties-> call add_edge -> call recalculate
// -> display file
//                                                  ^Nimit            ^Gogo
/*
TBD:
...
- Add_edge (Nimit)
- Recalculate (Gogo)
- Time funtionality in Display.h and final formatting to match display style
(Nimit)
...
int main(int argc, char *argv[]) {
	...
	display_sheet(
	    sheet); // Display the sheet (assuming this function is defined)
	clock_t en = clock();
	...
	while (1) {
		...
		if (strcmp(input, "q") == 0) {
			...
			// printf("%d\n", strlen(input));
			int result = parseInput(
			    input,
			    sheet); // Assuming parse_input is implemented in parser.h
			if (result != 1) {
				suc = 0;
			}
		}
